...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    if (openBrackets.indexOf(list[i]) !== -1) {
      stack.push(list[i]);
    } else if (!stack.length || closeBrackets[openBrackets.indexOf(stack.pop())] !== list[i]) {
      return false;
    }
  }
  if (stack.length) {
...
function commonPrefixUtil(str1, str2) {
  ...
  for (let i = 0, j = 0; i <= n1 - 1 && j <= n2 - 1; i += 1, j += 1) {
    if (str1[i] !== str2[j]) {
      break;
    }
    buffer += str1[i];
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    prefix = commonPrefixUtil(prefix, pathes[i]);
  }
  return (prefix);
}
...
function evaluateTicTacToePosition(position) {
  const xRow = position.map((arr) => arr.filter((el) => el === 'X').length);
  if (xRow.indexOf(3) !== -1) {
    return 'X';
  }
  const xColumn = [0, 0, 0];
  ...
  const oRow = position.map((arr) => arr.filter((el) => el === '0').length);
  if (oRow.indexOf(3) !== -1) {
    return '0';
  }
  const oColumn = [0, 0, 0];
